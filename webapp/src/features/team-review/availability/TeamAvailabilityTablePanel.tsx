 ...
 */
export function TeamAvailabilityTablePanel(
  props: TeamAvailabilityTablePanelProps,
): ReactElement {
  const rows = sortByNewest(props.rows, (row) =>
    ...
    <article className="cf:grid cf:gap-4 cf:rounded-2xl cf:border cf:border-white/10 cf:bg-black/20 cf:p-4 cf:lg:grid-cols-[1fr_14rem]">
      <div className="cf:min-w-0">
        ...
        <div className="cf:mt-3 cf:flex cf:flex-wrap cf:gap-2">
          <AvailabilityChip
            label="Duration"
            value={formatLeaveDuration(request)}
          />
