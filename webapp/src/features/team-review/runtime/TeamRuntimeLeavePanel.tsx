 ...
 */
export function TeamRuntimeLeavePanel(
  props: TeamRuntimeLeavePanelProps,
): ReactElement {
  const approvedLeaves = sortByNewest(props.approvedLeaves, (row) =>
    newestLeaveDateValue(row.leaveRequest),
  );
    ...
    <article className="cf:grid cf:gap-4 cf:rounded-2xl cf:border cf:border-white/10 cf:bg-black/20 cf:p-4 cf:lg:grid-cols-[1fr_auto]">
      <div className="cf:min-w-0">
        ...
        <p className="cf:mt-2 cf:text-sm cf:font-semibold cf:text-muted-foreground">
          {props.row.leaveTypeName} · {formatLeaveRange(props.row)} ·{" "}
          {formatLeaveDuration(props.row)}
        </p>
